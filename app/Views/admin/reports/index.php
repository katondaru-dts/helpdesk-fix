...
<style>
    ...
    .<?php echo has_permission('Ekspor Data') ? 'btn-cetak' : 'btn-cetak-hidden'; ?> {
        padding: 8px 16px;
        border-radius: 8px;
    ...
    .<?php echo has_permission('Ekspor Data') ? 'btn-cetak' : 'btn-cetak-hidden'; ?>:hover {
        background: #f1f5f9;
    }

    .<?php echo has_permission('Ekspor Data') ? 'btn-excel' : 'btn-excel-hidden'; ?> {
        padding: 8px 16px;
        border-radius: 8px;
        border: none;
        background: #059669;
    ...
    .<?php echo has_permission('Ekspor Data') ? 'btn-excel' : 'btn-excel-hidden'; ?>:hover {
        background: #047857;
    }

    .<?php echo has_permission('Ekspor Data') ? 'btn-pdf' : 'btn-pdf-hidden'; ?> {
        padding: 8px 16px;
        border-radius: 8px;
        border: none;
        background: #dc2626;
    ...
    .<?php echo has_permission('Ekspor Data') ? 'btn-pdf' : 'btn-pdf-hidden'; ?>:hover {
        background: #b91c1c;
    }
...
<div style="display: flex; align-items: flex-start; justify-content: space-between; margin-bottom: 24px; flex-wrap: wrap; gap: 12px;"
    ...
    <div>
        ...
        <p style="font-size: 13px; color: #64748b; margin: 0;">Monitor performa IT Support dan ringkasan data tiket
            secara keseluruhan</p>
    </div>
    <div style="display: flex; align-items: center; gap: 8px; flex-wrap: wrap;">
        <?php if (has_permission('Ekspor Data')): ?>
            <button onclick="window.print()"
                class="<?php echo has_permission('Ekspor Data') ? 'btn-cetak' : 'btn-cetak-hidden'; ?>"><i
                    class="bi bi-printer"></i> Cetak</button>
            <a href="<?= base_url('admin/reports/excel') ?>?f-from=<?= esc($dateFrom) ?>&f-to=<?= esc($dateTo) ?>"
                class="<?php echo has_permission('Ekspor Data') ? 'btn-excel' : 'btn-excel-hidden'; ?>"><i
                    class="bi bi-file-earmark-excel"></i> Excel</a>
            <a href="<?= base_url('admin/reports/pdf') ?>?f-from=<?= esc($dateFrom) ?>&f-to=<?= esc($dateTo) ?>"
                class="<?php echo has_permission('Ekspor Data') ? 'btn-pdf' : 'btn-pdf-hidden'; ?>"><i
...
<div class="filter-card no-print">
    <form action="<?= base_url('admin/reports') ?>" method="GET"
        ...
        <div style="flex: 1; min-width: 180px;">
            <label
                style="display: block; font-size: 12px; font-weight: 700; color: #475569; margin-bottom: 6px; text-transform: uppercase; letter-spacing: 0.05em;">Dari
                Tanggal</label>
            <input type="date" name="f-from" value="<?= esc($dateFrom) ?>"
                style="width: 100%; padding: 9px 12px; border: 1.5px solid #cbd5e1; border-radius: 8px; font-size: 13px; color: #334155; background: white; outline: none; box-sizing: border-box;">
        </div>
        <div style="flex: 1; min-width: 180px;">
            <label
                style="display: block; font-size: 12px; font-weight: 700; color: #475569; margin-bottom: 6px; text-transform: uppercase; letter-spacing: 0.05em;">Sampai
                Tanggal</label>
            <input type="date" name="f-to" value="<?= esc($dateTo) ?>"
                style="width: 100%; padding: 9px 12px; border: 1.5px solid #cbd5e1; border-radius: 8px; font-size: 13px; color: #334155; background: white; outline: none; box-sizing: border-box;">
        </div>
...
<div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 20px; margin-bottom: 28px;">
    <div class="report-stat-card">
        <div class="report-stat-icon" style="background: #dbeafe; color: #2563eb;"><i
    ...
    <div class="report-stat-card">
        <div class="report-stat-icon" style="background: #fef3c7; color: #d97706;"><i class="bi bi-hourglass-split"></i>
        </div>
        <div>
    ...
    <div class="report-stat-card">
        <div class="report-stat-icon" style="background: #d1fae5; color: #059669;"><i
    ...
    <div class="report-stat-card">
        <div class="report-stat-icon" style="background: #fee2e2; color: #dc2626;"><i
        ...
        <table class="report-table">
            <thead>
                <tr>
                    ...
                    <th style="width: 200px;">Judul Tiket</th>
                    <th style="text-align: center; width: 80px;">Prioritas</th>
                    ...
                    <th style="text-align: right; padding-right: 24px; width: 90px;">Tanggal</th>
                </tr>
            </thead>
            <tbody>
                    ...
                    <?php foreach ($tickets as $t): ?>
                        <tr>
                            <td style="padding-left: 24px;">
                                <span
                                    style="font-family: monospace; font-size: 12px; font-weight: 700; color: #2563eb; background: #dbeafe; padding: 2px 8px; border-radius: 6px;"><?= esc($t['id']) ?></span>
                            </td>
                            <td>
                                <div style="font-weight: 600; color: #1e293b; font-size: 13.5px;"><?= esc($t['title']) ?></div>
                            </td>
                            ...
                            <td style="text-align: center;">
                                <span
                                    class="status-badge sta-<?= strtoupper(str_replace(' ', '_', $t['status'])) ?>"><?= $t['status'] ?></span>
                            </td>
                            <td>
                                <div style="font-size: 13px; color: #475569; font-weight: 500;"><?= esc($t['reporter_name']) ?>
                                </div>
                            </td>
                            <td>
                            ...
                            <td>
                                <?php
                                // display_link berisi presigned URL (hanya untuk tombol buka, jangan disimpan ke DB)
                                $displayLink = !empty($t['display_link']) ? explode("\n", $t['display_link'])[0] : '';
                                $openLink = !empty($t['drive_link']) ? $t['drive_link'] : $displayLink;
                                ?>
                                <form action="<?= base_url('admin/reports/update-link/' . $t['id']) ?>" method="POST"
                                    ...
                                    <?php if (!empty($openLink)): ?>
                                        <a href="<?= esc($openLink) ?>" target="_blank" class="link-open-btn" title="Buka Link"><i
                            ...
                            <td style="text-align: right; padding-right: 24px;">
                                <span
                                    style="font-size: 12px; color: #64748b; font-weight: 500; white-space: nowrap;"><?= date('d/m/Y', strtotime($t['created_at'])) ?></span>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                <?php endif; ?>
